...
from MM_VMware import OUTPUT_COLUMN_ORDER
from web_app import App, make_server
...
class WebTests(unittest.TestCase):
    ...
    def test_upload_process_download_and_validation_error(self):
        ...
        self.upload('target', pd.DataFrame({'name': ['a', 'b', 'c'],
                                          'maintenance_state': ['inMaintenance', 'inMaintenance', 'notInMaintenance'],
                                          'connection_state': ['connected'] * 3}), '.csv')
        state = self.status()
        self.assertTrue(all(f['ready'] for f in state['files'].values()))
        for role, directory in [('rsm', 'rsm'), ('pu', 'PU'), ('target', 'data')]:
        ...
        before = pd.read_excel(next((self.root / 'log').glob('*.xlsx')))
        self.assertEqual(len(before), 2)
        self.assertTrue(next((self.root / 'output').glob('*.xlsx')).name.startswith('MM_'))
        self.upload('target', pd.DataFrame({'wrong_column': [1]}), '.csv')
